...
===
This is step to step guide for creating packages for running ipfs on handheld devices and sharing files b/w multiple different platforms. Here we will be focusing on creating android and ios applications including all the required steps that are absent in the original package, in their platform distribution file types.

This method is based on the package that is available as an open-source here -> https://github.com/ipfs-shipyard/gomobile-ipfs .

This will also guide you through how we can use a customized goipfs repository and implement it in our distributions.
Here is the original repo -> https://github.com/ipfs-shipyard/gomobile-ipfs

PS: this method of creation is specifically based on the experience and how we individually were successfully able to create them. By no means this is the only way to complete the work.
...
## Linux Setup
the following commands are to be run in a linux system. this may also be possible to work on using a VirtualBox, just remember you need to have at least 20GB of free space to get your work done.

This was previously done in Ubuntu LTS, so the guide is tuned according to that.
...
- to get **make and gcc** [ all other basic stuff ] in case you don't have them in your linux system run the following commands in the terminal.
    ```
...
- **setting up android sdk.** This is easy to do in a linux system. after installation add this directory to path in ~/.profile or ~/.bash under the alias of `ANDROID_HOME` . 
    ...
    ```
- **setting up android ndk** This is a bit tricky. 
    
    First, download the android ndk from this place. -> [android ndk](https://developer.android.com/ndk/downloads) 

    The one we used during the build process was `android-ndk-r23b` . To download the specific version the option is available [here](https://dl.google.com/android/repository/android-ndk-r23b-linux.zip).

    Once downloaded unzip the content and move it to the same directory where you have your android sdk installed. i.e; in /usr/lib
...
- **Installing go** . for our specific task we need to install go in our system. The latest package won't do as most of the modules we are going to use are not yet compatible with anything higher than go 1.17 at the moment.
 Therefore we will install go 1.17.
    ...
    next, we will add it to path directly. no alias is needed. which will look something like this in ~/.profile
    ```
    export PATH=$PATH:/usr/local/go/bin
    ```
Now we have completed the environment setup and we can move to the next step.
...
**<ins>(or)<ins>**
 ...
 we can use it in collaboration with our custom goipfs package to also include custom commands and operations.
...
This will create a directory with the name 'build'. And our aar file will be inside '/android/intermediates/core' which will be named `core.aar`.

<div style="background:#222; padding:20px 20px 20px 20px; margin:10px 10px 10px 10px;">
NOTE : There are occasions where the building process fails sometimes, so we need to do one more thing in case we are hit with an error when this command is run.
...
You will find a ‘go’ folder, and open it.
...
The generated application doesn't run on marshmallow anymore.
```
version: android 24+
name: oreo 
minimum memory: 2 GB
...
But still for situations where you need to turn the original project into a workable build follow the guide [here](./docs/docs/gomobile-ipfs-fix.pdf).
